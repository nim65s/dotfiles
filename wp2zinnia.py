...
from zinnia.signals import disconnect_discussion_signals, disconnect_entry_signals
...
class Command(LabelCommand):
    ...
    def import_categories(self, category_nodes):
        ...
        for category_node in category_nodes:
            ...
            category, created = Category.objects.get_or_create(
                slug=slug, defaults={'title': title.encode('ascii', 'xmlcharrefreplace'),
    ...
    def import_tags(self, tag_nodes):
        ...
        for tag_node in tag_nodes:
            ...
            Tag.objects.get_or_create(name=tag_name.encode('ascii', 'xmlcharrefreplace'))
            self.write_out(self.style.ITEM('OK\n'))
    ...
    def import_image(self, entry, items, image_id):
        for item in items:
            ...
            if (post_type == 'attachment' and
                ...
                img_tmp.flush()
                entry.image.save(os.path.basename(image_url[:100]),
                                 File(img_tmp))
                self.write_out(self.style.ITEM('OK\n'))
